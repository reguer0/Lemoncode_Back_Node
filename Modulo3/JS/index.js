...
/**
 * Ejercicio 2
Implementar una funcion replaceAt que tome como primer argumento un array, 
como segundo argumento un índice y como tercer argumento un valor y reemplace el elemento 
dentro del array en el índice indicado. El array de entrada no debe de ser mutado, eso es, 
que debes crear un nuevo array sin modificar el existente. Utiliza spread operator, y slice para conseguirlo.
 */
...
const winnerByYear = (arrLumnos, year) => {
   const alumnos = arrLumnos.filter(person => person.year === year);
   return alumnos
...
console.log(winnerByYear(arrLumnos, 1998)) // [ 'Douglass', 'Randy', 'Monroe' ]

/** 
*Ejercicio 4
    Crear funcion para normalizar una colección de objetos a un objeto, 
    de tal manera que devuelva un objeto que tenga como claves las ids de los objetos 
    y como valores los objetos en sí pero sin la id.
*/
const collection = [
  {
...
const normalize = (arr) => {
    ...
    arr.forEach(({id, ...rest}) => {
        normalizedObj[id] = {...rest};
    });
  return normalizedObj;
};

const resultNormalized = normalize(collection);
...
Implementa una función para eliminar valores falsys de una estructura de datos.
 ...
 Siempre deberá de crear una estructura nueva. 
...
const compact = (arg) => {
  // Si no es objeto ni array, devolver el argumento tal cual
